...
@TriggerSerially
@CapabilityDescription("consume events from an opc ua server")
@SeeAlso({})
@ReadsAttributes({ @ReadsAttribute(attribute = "", description = "") })
@WritesAttributes({ @WritesAttribute(attribute = "", description = "") })
public class ConsumeOPCEvents extends AbstractProcessor {

    public static final PropertyDescriptor OPC_SERVER_URL = new PropertyDescriptor.Builder().name("OPC_SERVER_URL")
            .displayName("SERVER URL for OPC").description("opc.tcp://host:port/server").required(true)
            .expressionLanguageSupported(true).addValidator(StandardValidators.NON_EMPTY_VALIDATOR).build();
    public static final PropertyDescriptor NAME_SPACE_INDEX = new PropertyDescriptor.Builder().name("NameSpaceIndex")
            .displayName("Name Space Index for the identifier").description("").required(true)
            .addValidator(StandardValidators.INTEGER_VALIDATOR).build();

    public static final PropertyDescriptor IDENTIFIER = new PropertyDescriptor.Builder().name("Identifier")
            .displayName("Identifier for the metric").description("").required(false)
            .addValidator(StandardValidators.CHARACTER_SET_VALIDATOR).build();

    public static final PropertyDescriptor SUB_DUR = new PropertyDescriptor.Builder().name("SUB_DUR")
            .displayName("Duration for the subscription, how long to listen for response").description("")
            .required(true).addValidator(StandardValidators.LONG_VALIDATOR).build();
    ...
    public void onScheduled(final ProcessContext context) {
        ...
        try {
            ...
            client.connect().get();
            double sub_dur = context.getProperty(SUB_DUR).asDouble();
            UaSubscription subscription = client.getSubscriptionManager().createSubscription(sub_dur).get();
            ...
            Integer nsIndex = context.getProperty(NAME_SPACE_INDEX).asInteger();
            browseNode("", client, Identifiers.RootFolder, nodes, nsIndex);
            for (NodeId node : nodes) {
                ReadValueId readValueId = new ReadValueId(node, AttributeId.Value.uid(), null,
            ...
            for (UaMonitoredItem item : items) {
                if (item.getStatusCode().isGood()) {
                    getLogger().info("item created for nodeId={}", new Object[] { item.getReadValueId().getNodeId() });
                } else {
                    getLogger().warn("failed to create item for nodeId={} (status={})",
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        try {
            while (isScheduled()) {
                HashMap<String, Object> dataPoint = dataPoints.poll(5, TimeUnit.SECONDS);
                if (dataPoint != null) {
                    FlowFile flowFile = session.create();
                    JSONObject data = new JSONObject(dataPoint);
                    if (data.toJSONString().length() > 0) {
                        getLogger().error(data.toJSONString());
    ...
    private OpcUaClient createClient(String url) throws Exception {
        ...
        OpcUaClientConfig config = OpcUaClientConfig.builder()
                ...
                .setEndpoint(endpoint).setMaxResponseMessageSize(uint(10)).setIdentityProvider(new AnonymousProvider())
                .setRequestTimeout(uint(5000000)).build();
    ...
    private void onSubscriptionValue(UaMonitoredItem item, DataValue value) {
        ...
        // item="+item.getReadValueId().getNodeId()+",
    ...
    private boolean browseNode(String indent, OpcUaClient client, NodeId browseRoot, List<NodeId> nodes,
            Integer nsIndex) {
        try {
            boolean isLeaf = false;
            boolean hasChild = false;
            List<Node> childNodes = client.getAddressSpace().browse(browseRoot).get();
            hasChild = childNodes.size() > 0;
            for (Node node : childNodes) {
                // recursively browse to children
                isLeaf = browseNode(indent + "  ", client, node.getNodeId().get(), nodes, nsIndex);
                if (isLeaf) {
                    if (node.getNodeId().get().getNamespaceIndex().intValue() == nsIndex)
                        nodes.add(node.getNodeId().get());
                }
            }
            return !hasChild;
        } catch (InterruptedException | ExecutionException e) {
            getLogger().error("Browsing nodeId={} failed: {}" + browseRoot + e.getMessage() + e);
            return false;
        }
    }
